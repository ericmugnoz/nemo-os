...
AsmToken asm_lexer_next(AsmLexer *lx) {
    ...
    if (c == '#' || nb_isdigit(c) || (c == '-' && nb_isdigit(peek2(lx)))) {
        bool had_hash = (c == '#');
        ...
        if (peek(lx) == '0' && (peek2(lx) == 'x' || peek2(lx) == 'X')) {
            ...
            while (nb_isxdigit(peek(lx))) {
                char h = advance(lx);
                int d = (h >= '0' && h <= '9') ? h - '0' : (nb_tolower(h) - 'a' + 10);
                value = value * 16 + d;
            }
        } else {
            while (nb_isdigit(peek(lx))) {
        ...
        if (!had_hash && !neg) {
            if (peek(lx) == ':') {
                ...
                nb_memcpy(tok.text, lx->src + start_digits, (uint32_t)len);
                tok.text[len] = '\0';
                return tok;
            }
            if ((peek(lx) == 'f' || peek(lx) == 'b') && !nb_isalnum(peek2(lx))) {
    ...
    if (nb_isalpha(c) || c == '_' || (c == '.' && peek2(lx) == 'L')) {
        int start = lx->pos;
        if (c == '.') advance(lx); // el punto inicial de ".Lxxx"
        while (nb_isalnum(peek(lx)) || peek(lx) == '_' || peek(lx) == '$') advance(lx);
    ...
    if (c == '.') {
        advance(lx);
        int start = lx->pos;
        while (nb_isalnum(peek(lx)) || peek(lx) == '.' || peek(lx) == '_') advance(lx);
        int len = lx->pos - start;
        if (len >= ASM_TOKEN_TEXT_MAX) len = ASM_TOKEN_TEXT_MAX - 1;
        nb_memcpy(tok.text, lx->src + start, (uint32_t)len);
        tok.text[len] = '\0';
