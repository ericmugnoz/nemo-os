...
static int find_symbol(const char *name) {
    for (int i = 0; i < symbol_count; i++) {
        if (nb_strcmp(symbols[i].name, name) == 0) return i;
    }
    return -1;
}
...
static void define_symbol(const char *name, int section, uint32_t offset) {
    if (find_symbol(name) >= 0) {
        nb_fatal(0, "etiqueta definida mas de una vez", name);
    }
    if (symbol_count >= MAX_SYMBOLS) { nb_fatal(0, "demasiadas etiquetas", NULL); }
    nb_strncpy(symbols[symbol_count].name, name, 63);
...
static uint32_t symbol_address(const char *name) {
    ...
    if (idx < 0) { nb_fatal(0, "simbolo no definido", name); }
...
static int reg_number(const char *name) {
    if (nb_strcmp(name, "sp") == 0) return REG_SP;
    if (nb_strcmp(name, "xzr") == 0 || nb_strcmp(name, "wzr") == 0) return REG_ZR;
    if ((name[0] == 'x' || name[0] == 'w' || name[0] == 'd' || name[0] == 's') && name[1] != '\0') {
        return (int)nb_atoi64(name + 1);
    }
    nb_fatal(0, "registro no reconocido", name);
}

// ---- condiciones ----

static int cond_from_name(const char *name) {
    if (nb_strcmp(name, "eq") == 0) return COND_EQ;
    if (nb_strcmp(name, "ne") == 0) return COND_NE;
    if (nb_strcmp(name, "cs") == 0 || nb_strcmp(name, "hs") == 0) return COND_CS;
    if (nb_strcmp(name, "cc") == 0 || nb_strcmp(name, "lo") == 0) return COND_CC;
    if (nb_strcmp(name, "mi") == 0) return COND_MI;
    if (nb_strcmp(name, "pl") == 0) return COND_PL;
    if (nb_strcmp(name, "vs") == 0) return COND_VS;
    if (nb_strcmp(name, "vc") == 0) return COND_VC;
    if (nb_strcmp(name, "hi") == 0) return COND_HI;
    if (nb_strcmp(name, "ls") == 0) return COND_LS;
    if (nb_strcmp(name, "ge") == 0) return COND_GE;
    if (nb_strcmp(name, "lt") == 0) return COND_LT;
    if (nb_strcmp(name, "gt") == 0) return COND_GT;
    if (nb_strcmp(name, "le") == 0) return COND_LE;
    nb_fatal(0, "condicion no reconocida", name);
}
...
static void aexpect(AParser *p, AsmTokenType t, const char *what) {
    if (p->cur.type != t) {
        nb_fatal(p->cur.line, "se esperaba", what);
    }
    aadvance(p);
}
...
static int parse_reg(AParser *p) {
    if (p->cur.type != AT_IDENT) {
        nb_fatal(p->cur.line, "se esperaba un registro", NULL);
    }
...
typedef struct {
    ...
    bool reg_offset;     // true si el desplazamiento era un REGISTRO, [rn, rm]
    int rm;               // el registro de desplazamiento, si reg_offset
} MemOperand;

static MemOperand parse_mem_operand(AParser *p) {
    MemOperand m;
    nb_memset(&m, 0, sizeof(m));
    aexpect(p, AT_LBRACKET, "'['");
    m.rn = parse_reg(p);
    if (p->cur.type == AT_COMMA) {
        aadvance(p);
        if (p->cur.type == AT_IDENT) {
            // [rn, rm] -- desplazamiento por REGISTRO, no inmediato
            m.reg_offset = true;
            m.rm = parse_reg(p);
            ...
            nb_fatal(p->cur.line, "se esperaba un numero o un registro en el desplazamiento", NULL);
        }
    }
    ...
    if (p->cur.type == AT_BANG) {
        ...
        if (p->cur.type != AT_NUMBER) {
            nb_fatal(p->cur.line, "se esperaba un numero tras ','", NULL);
        }
        m.imm = (int32_t)p->cur.num_value;
        aadvance(p);
...
static void encode_mnemonic(AParser *p, const char *mn) {
    if (nb_strcmp(mn, "mov") == 0) {
        bool dst_is_sp = (p->cur.type == AT_IDENT && nb_strcmp(p->cur.text, "sp") == 0);
        int rd = parse_reg(p);
        aexpect(p, AT_COMMA, "','");
        if (p->cur.type == AT_NUMBER) {
            ...
            bool src_is_sp = (p->cur.type == AT_IDENT && nb_strcmp(p->cur.text, "sp") == 0);
            int rn = parse_reg(p);
    ...
    if (nb_strcmp(mn, "add") == 0 || nb_strcmp(mn, "sub") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
        bool is_add = (mn[0] == 'a');
        if (p->cur.type == AT_LO12) {
            aadvance(p);
            if (p->cur.type != AT_IDENT) { nb_fatal(p->cur.line, "se esperaba un simbolo tras :lo12:", NULL); }
            ...
            if (p->cur.type == AT_COMMA) {
                aadvance(p);
                if (p->cur.type != AT_IDENT || nb_strcmp(p->cur.text, "lsl") != 0) {
                    nb_fatal(p->cur.line, "solo se soporta 'lsl' como desplazamiento", NULL);
                }
                aadvance(p);
                if (p->cur.type != AT_NUMBER) { nb_fatal(p->cur.line, "se esperaba #cantidad", NULL); }
    ...
    if (nb_strcmp(mn, "subs") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
        if (p->cur.type == AT_NUMBER) {
            if (p->cur.num_value < 0) {
                nb_fatal(p->cur.line, "'subs' con inmediato negativo no soportado", NULL);
            }
            uint32_t imm = (uint32_t)p->cur.num_value;
            aadvance(p);
            emit_word(enc_subs_imm(rd, rn, imm));
        } else {
            int rm = parse_reg(p);
    ...
    if (nb_strcmp(mn, "cmp") == 0) {
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
        if (p->cur.type == AT_NUMBER) {
            if (p->cur.num_value < 0) {
                nb_fatal(p->cur.line, "'cmp' con inmediato negativo no soportado", NULL);
            }
            uint32_t imm = (uint32_t)p->cur.num_value;
            aadvance(p);
    ...
    if (nb_strcmp(mn, "mul") == 0 || nb_strcmp(mn, "sdiv") == 0 || nb_strcmp(mn, "udiv") == 0 ||
        nb_strcmp(mn, "and") == 0 || nb_strcmp(mn, "orr") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
        if ((nb_strcmp(mn, "and") == 0 || nb_strcmp(mn, "orr") == 0) && p->cur.type == AT_NUMBER) {
            uint64_t imm = (uint64_t)p->cur.num_value;
            aadvance(p);
            bool is_and = (nb_strcmp(mn, "and") == 0);
            uint32_t word = is_and ? enc_and_imm(rd, rn, imm) : enc_orr_imm(rd, rn, imm);
            if (word == 0) {
                char imm_str[24];
                nb_itoa((int64_t)imm, imm_str, sizeof(imm_str));
                nb_fatal(p->cur.line, is_and ? "'and' con este inmediato no soportado (solo mascaras de bits bajos contiguos)" : "'orr' con este inmediato no soportado (solo mascaras de bits bajos contiguos)", imm_str);
            }
            emit_word(word);
            return;
        }
        int rm = parse_reg(p);
        if (nb_strcmp(mn, "mul") == 0) emit_word(enc_mul(rd, rn, rm));
        else if (nb_strcmp(mn, "sdiv") == 0) emit_word(enc_sdiv(rd, rn, rm));
        else if (nb_strcmp(mn, "udiv") == 0) emit_word(enc_udiv(rd, rn, rm));
        else if (nb_strcmp(mn, "and") == 0) emit_word(enc_and_reg(rd, rn, rm));
        else emit_word(enc_orr_reg(rd, rn, rm));
        return;
    }
    if (nb_strcmp(mn, "msub") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
    ...
    if (nb_strcmp(mn, "neg") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p);
        emit_word(enc_sub_reg(rd, REG_ZR, rn));
        return;
    }
    if (nb_strcmp(mn, "lsl") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
        if (p->cur.type == AT_NUMBER) {
    ...
    if (nb_strcmp(mn, "fmov") == 0) {
        if (p->cur.type != AT_IDENT) { nb_fatal(p->cur.line, "se esperaba un registro", NULL); }
        ...
        if (dst_is_fp && !src_is_fp) {
            ...
            nb_fatal(p->cur.line, "fmov necesita mezclar un registro FP (d/s) con uno general (x/w)", NULL);
        }
        return;
    }
    if (nb_strcmp(mn, "scvtf") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p);
        emit_word(enc_scvtf(rd, rn));
        return;
    }
    if (nb_strcmp(mn, "fcvtzs") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p);
        emit_word(enc_fcvtzs(rd, rn));
        return;
    }
    if (nb_strcmp(mn, "fadd") == 0 || nb_strcmp(mn, "fsub") == 0 || nb_strcmp(mn, "fmul") == 0 || nb_strcmp(mn, "fdiv") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rm = parse_reg(p);
        if (nb_strcmp(mn, "fadd") == 0) emit_word(enc_fadd(rd, rn, rm));
        else if (nb_strcmp(mn, "fsub") == 0) emit_word(enc_fsub(rd, rn, rm));
        else if (nb_strcmp(mn, "fmul") == 0) emit_word(enc_fmul(rd, rn, rm));
        else emit_word(enc_fdiv(rd, rn, rm));
        return;
    }
    if (nb_strcmp(mn, "fcmp") == 0) {
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rm = parse_reg(p);
        emit_word(enc_fcmp(rn, rm));
        return;
    }
    if (nb_strcmp(mn, "fneg") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p);
        emit_word(enc_fneg(rd, rn));
        return;
    }
    if (nb_strcmp(mn, "fabs") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p);
        emit_word(enc_fabs(rd, rn));
        return;
    }
    if (nb_strcmp(mn, "fsqrt") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p);
        emit_word(enc_fsqrt(rd, rn));
        return;
    }
    if (nb_strcmp(mn, "frintm") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p);
        emit_word(enc_frintm(rd, rn));
        return;
    }
    if (nb_strcmp(mn, "frintp") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p);
        emit_word(enc_frintp(rd, rn));
        return;
    }
    if (nb_strcmp(mn, "fcvt") == 0) {
        int rd = parse_reg(p);
        aexpect(p, AT_COMMA, "','");
    ...
    if (nb_strcmp(mn, "cset") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        if (p->cur.type != AT_IDENT) { nb_fatal(p->cur.line, "se esperaba una condicion", NULL); }
        int cond = cond_from_name(p->cur.text);
        aadvance(p);
        emit_word(enc_cset(rd, cond));
        return;
    }
    if (nb_strcmp(mn, "cneg") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
        if (p->cur.type != AT_IDENT) { nb_fatal(p->cur.line, "se esperaba una condicion", NULL); }
        int cond = cond_from_name(p->cur.text);
    ...
    if (nb_strcmp(mn, "ret") == 0) { emit_word(enc_ret()); return; }
    if (nb_strcmp(mn, "br") == 0) { int rn = parse_reg(p); emit_word(enc_br(rn)); return; }
    if (nb_strcmp(mn, "svc") == 0) {
        if (p->cur.type != AT_NUMBER) { nb_fatal(p->cur.line, "se esperaba #inmediato", NULL); }
        uint32_t imm = (uint32_t)p->cur.num_value;
        aadvance(p);
        emit_word(enc_svc(imm));
        return;
    }
    if (nb_strcmp(mn, "b") == 0 || nb_strcmp(mn, "bl") == 0) {
        if (p->cur.type != AT_IDENT) { nb_fatal(p->cur.line, "se esperaba una etiqueta", NULL); }
    ...
    if (nb_strcmp(mn, "cbz") == 0 || nb_strcmp(mn, "cbnz") == 0) {
    ...
    if (nb_strcmp(mn, "adrp") == 0) {
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        if (p->cur.type != AT_IDENT) { nb_fatal(p->cur.line, "se esperaba un simbolo", NULL); }
        uint32_t target = symbol_address(p->cur.text);
        aadvance(p);
    ...
    if (nb_strcmp(mn, "ldr") == 0 || nb_strcmp(mn, "str") == 0 || nb_strcmp(mn, "ldrb") == 0 || nb_strcmp(mn, "strb") == 0) {
        int rt = parse_reg(p); aexpect(p, AT_COMMA, "','");
        MemOperand m = parse_mem_operand(p);
        bool is_byte = (mn[3] == 'b');
        bool is_load = (mn[0] == 'l');
        if (m.reg_offset) {
            if (is_byte) emit_word(is_load ? enc_ldrb_reg(rt, m.rn, m.rm) : enc_strb_reg(rt, m.rn, m.rm));
            else emit_word(is_load ? enc_ldr_reg(rt, m.rn, m.rm) : enc_str_reg(rt, m.rn, m.rm));
        } else if (m.writeback_pre) {
    ...
    if (nb_strcmp(mn, "stp") == 0 || nb_strcmp(mn, "ldp") == 0) {
    ...
    nb_fatal(p->cur.line, "instruccion no soportada", mn);
}
...
static void pass1_scan(const char *source) {
    ...
    while (p.cur.type != AT_EOF) {
        ...
        for (;;) {
            ...
            char name[64]; nb_strncpy(name, p.cur.text, 63); name[63] = '\0';
            aadvance(&p);
        ...
        if (p.cur.type == AT_DIRECTIVE) {
            char dir[64]; nb_strncpy(dir, p.cur.text, 63); dir[63] = '\0';
            aadvance(&p);
            if (nb_strcmp(dir, "section") == 0) {
                if (p.cur.type != AT_DIRECTIVE) { nb_fatal(p.cur.line, "se esperaba el nombre de la seccion", NULL); }
                if (nb_strcmp(p.cur.text, "text.start") == 0 || nb_strcmp(p.cur.text, "text") == 0) section = SEC_TEXT;
                else if (nb_strcmp(p.cur.text, "rodata") == 0) section = SEC_RODATA;
                else if (nb_strcmp(p.cur.text, "bss") == 0) section = SEC_BSS;
                else { nb_fatal(p.cur.line, "seccion no reconocida", p.cur.text); }
                aadvance(&p);
            } else if (nb_strcmp(dir, "global") == 0) {
                if (p.cur.type == AT_IDENT) aadvance(&p);
            } else if (nb_strcmp(dir, "asciz") == 0) {
                if (p.cur.type != AT_STRING) { nb_fatal(p.cur.line, "se esperaba una cadena", NULL); }
                sec_size[section] += (uint32_t)nb_strlen(p.cur.text) + 1;
                aadvance(&p);
            } else if (nb_strcmp(dir, "space") == 0) {
                if (p.cur.type != AT_NUMBER) { nb_fatal(p.cur.line, "se esperaba un numero", NULL); }
                sec_size[section] += (uint32_t)p.cur.num_value;
                aadvance(&p);
            } else if (nb_strcmp(dir, "align") == 0) {
                if (p.cur.type != AT_NUMBER) { nb_fatal(p.cur.line, "se esperaba un numero", NULL); }
                uint32_t bytes = 1u << (uint32_t)p.cur.num_value;
                sec_size[section] = (sec_size[section] + bytes - 1) & ~(bytes - 1);
                aadvance(&p);
            } else if (nb_strcmp(dir, "quad") == 0) {
                if (p.cur.type != AT_NUMBER && p.cur.type != AT_IDENT) {
                    nb_fatal(p.cur.line, "se esperaba un numero o una etiqueta tras .quad", NULL);
                }
                sec_size[section] += 8;
                aadvance(&p);
            } else {
                nb_fatal(p.cur.line, "directiva no soportada", dir);
            }
        } else if (p.cur.type == AT_IDENT) {
            char mn[64]; nb_strncpy(mn, p.cur.text, 63); mn[63] = '\0';
            aadvance(&p);
            if (nb_strcmp(mn, "mov") == 0) {
            ...
            nb_fatal(p.cur.line, "token inesperado al principio de linea", NULL);
        }
    }
...
static void pass2_encode(const char *source) {
    ...
    while (p.cur.type != AT_EOF) {
        ...
        if (p.cur.type == AT_DIRECTIVE) {
            ...
            aadvance(&p);
            if (nb_strcmp(dir, "section") == 0) {
                if (nb_strcmp(p.cur.text, "text.start") == 0 || nb_strcmp(p.cur.text, "text") == 0) {
                    section = SEC_TEXT; g_current_addr = g_text_base + g_text_pos;
                } else if (nb_strcmp(p.cur.text, "rodata") == 0) {
                ...
                uint32_t len = (uint32_t)nb_strlen(p.cur.text) + 1;
                nb_memcpy(g_rodata_out + g_rodata_pos, p.cur.text, len);
                g_rodata_pos += len;
                aadvance(&p);
            } else if (nb_strcmp(dir, "space") == 0) {
...
int64_t assemble(const char *source, uint8_t *out_buf, uint32_t out_buf_size) {
    ...
    if (g_out_text_size + g_out_rodata_size > out_buf_size) {
        char bytes_needed[12];
        nb_itoa(g_out_text_size + g_out_rodata_size, bytes_needed, sizeof(bytes_needed));
        nb_fatal(0, "el binario resultante no cabe en el buffer (bytes necesarios)", bytes_needed);
    }
    ...
    if (g_text_pos != g_out_text_size || g_rodata_pos != g_out_rodata_size) {
        nb_fatal(0, "error interno: la pasada 2 no coincide con el tamaño calculado en la pasada 1", NULL);
    }
