...
// que AND de inmediato (logical-immediate, misma mascara de bits
// bajos contiguos) -- solo cambia el campo 'opc' (bits 30:29) de 00
// (AND) a 01 (ORR), que equivale a sumar 0x20000000 a la base de AND.
...
uint32_t enc_asr_imm(int rd, int rn, int shift_amount) {
    // ASR xd,xn,#N es el alias de SBFM xd,xn,#N,#63 (con signo, a
    // diferencia de LSR que rellena con ceros)
    uint32_t immr = (uint32_t)(shift_amount & 63);
    uint32_t imms = 63u;
    return 0x93400000u | (immr << 16) | (imms << 10) | ((uint32_t)rn << 5) | (uint32_t)rd;
}

// Desplazamientos con la CANTIDAD en un registro (no inmediato) --
// hace falta para Shl/Shr/Sar de BlitzPlus, donde el numero de bits a
// desplazar puede ser una variable, no siempre una constante.
...
// EOR xd,xn,xm -- para el operador Xor de BlitzPlus.
...
uint32_t enc_br(int rn) { // br xn -- salto indirecto, para Return de Gosub
...
// (sin escala, opcion LSL/UXTX por defecto -- equivalente a like
// "[rn, rm]" tal cual, sin sufijo de extension explicito) ----
//
// Misma familia de instruccion que las de arriba (unsigned
// immediate), solo que con Rm+opcion en vez de imm12 -- verificado a
// mano bit a bit contra esas mismas codificaciones ya usadas en
// produccion (difieren exactamente donde el manual de arquitectura
// ARM64 predice: bit 24 distingue immediate/register, bit 21 fijo a
// 1 en la forma de registro).
...
//
// Todos los valores flotantes de Nemo-Blitz son 'double' de 64 bits
// -- no usamos precision simple en ningun sitio, asi que solo hace
// falta esta variante (type=01 en todas las codificaciones de abajo).

uint32_t enc_fmov_gpr_to_fpr(int rd, int rn) { // fmov dd, xn (bits, sin convertir)
...
uint32_t enc_fmov_gpr_to_fpr32(int rd, int rn) { // fmov sd, wn (variante de 32 bits)
    return 0x1E270000u | ((uint32_t)rn << 5) | (uint32_t)rd;
}
uint32_t enc_fmov_fpr_to_gpr32(int rd, int rn) { // fmov wd, sn (variante de 32 bits)
    return 0x1E260000u | ((uint32_t)rn << 5) | (uint32_t)rd;
}
uint32_t enc_scvtf(int rd, int rn) { // scvtf dd, xn (entero con signo -> double)
    return 0x9E620000u | ((uint32_t)rn << 5) | (uint32_t)rd;
}
uint32_t enc_fcvtzs(int rd, int rn) { // fcvtzs xd, dn (double -> entero con signo, trunca)
...
uint32_t enc_fneg(int rd, int rn) { // fneg dd, dn
    return 0x1E614000u | ((uint32_t)rn << 5) | (uint32_t)rd;
}
uint32_t enc_fabs(int rd, int rn) { // fabs dd, dn
    return 0x1E60C000u | ((uint32_t)rn << 5) | (uint32_t)rd;
}
uint32_t enc_fsqrt(int rd, int rn) { // fsqrt dd, dn
    return 0x1E61C000u | ((uint32_t)rn << 5) | (uint32_t)rd;
}
uint32_t enc_frintm(int rd, int rn) { // frintm dd, dn -- redondeo hacia -infinito (floor)
    return 0x1E654000u | ((uint32_t)rn << 5) | (uint32_t)rd;
}
uint32_t enc_frintp(int rd, int rn) { // frintp dd, dn -- redondeo hacia +infinito (ceil)
    return 0x1E64C000u | ((uint32_t)rn << 5) | (uint32_t)rd;
}
// FCVT: conversion de precision (simple<->doble) -- distinta de las
// FCVT* de redondeo, esta SOLO cambia el numero de bits usados.
// src_is_single indica el sentido: true = s->d (subimos precision),
// false = d->s (bajamos precision, puede perder informacion).
uint32_t enc_fcvt(int rd, int rn, int src_is_single) {
    if (src_is_single) return 0x1E22C000u | ((uint32_t)rn << 5) | (uint32_t)rd; // simple -> doble
    return 0x1E624000u | ((uint32_t)rn << 5) | (uint32_t)rd; // doble -> simple
}
