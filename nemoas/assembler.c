...
static MemOperand parse_mem_operand(AParser *p) {
    ...
    if (p->cur.type == AT_COMMA) {
        ...
        if (p->cur.type == AT_IDENT) {
            ...
            fprintf(stderr, "error (linea %d): se esperaba un numero o un registro en el desplazamiento\n", p->cur.line);
            exit(1);
        }
    }
...
static void encode_mnemonic(AParser *p, const char *mn) {
    ...
    if (strcmp(mn, "subs") == 0) {
        ...
        if (p->cur.type == AT_NUMBER) {
            if (p->cur.num_value < 0) {
                fprintf(stderr, "error (linea %d): 'subs' con inmediato negativo no soportado\n", p->cur.line);
                exit(1);
            }
    ...
    if (strcmp(mn, "mul") == 0 || strcmp(mn, "sdiv") == 0 || strcmp(mn, "udiv") == 0 ||
        ...
        int rd = parse_reg(p); aexpect(p, AT_COMMA, "','");
        int rn = parse_reg(p); aexpect(p, AT_COMMA, "','");
        if ((strcmp(mn, "and") == 0 || strcmp(mn, "orr") == 0) && p->cur.type == AT_NUMBER) {
            uint64_t imm = (uint64_t)p->cur.num_value;
            aadvance(p);
            bool is_and = (strcmp(mn, "and") == 0);
            uint32_t word = is_and ? enc_and_imm(rd, rn, imm) : enc_orr_imm(rd, rn, imm);
            if (word == 0) {
                fprintf(stderr, "error (linea %d): '%s' con #%llu no soportado "
                                "(solo mascaras de bits bajos contiguos)\n", p->cur.line, mn, (unsigned long long)imm);
                exit(1);
            }
            emit_word(word);
            return;
